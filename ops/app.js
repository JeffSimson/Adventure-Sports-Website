...
async function publish(e){e.preventDefault();const s=await ensure();if(!s)return;publishing=true;updatePreview();const button=$('#publishControlsButton');if(button){button.disabled=true;button.textContent='Publishing…'}try{const r=await fetch(PUBLISH,{method:'POST',headers:{Authorization:'Bearer '+s.token.access_token,'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify({fieldStatus:current(),announcement:$('#announcementInput').value.trim()})});const raw=await r.text();let d={};if(raw){try{d=JSON.parse(raw)}catch{d={error:raw.slice(0,300)}}}if(!r.ok)throw Error(d.error||d.message||`Publishing failed (${r.status}).`);if(!d.site)throw Error('The website updated, but the server did not return the updated site data. Refresh the page to confirm.');siteData=d.site;originalStatus=siteData.fieldStatus;originalAnnouncement=siteData.announcement;$('#publishNotice').textContent='Website status updated successfully.';$('#publishNotice').className='publish-notice success';$('#publishNotice').hidden=false;await loadSite()}catch(err){$('#publishNotice').textContent=err.message||'The website could not be updated.';$('#publishNotice').className='publish-notice error';$('#publishNotice').hidden=false}finally{publishing=false;if(button)button.textContent='Publish Changes';updatePreview()}}
