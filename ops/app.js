...
async function loadTrustedDevices(){const list=$('#securityTrustedDevicesList');if(!list||role()!=='owner')return;try{const d=await api('/.netlify/functions/security-devices');list.innerHTML=(d.devices||[]).map(x=>`<article class="trusted-device-card"><div class="trusted-device-icon">${x.operating_system==='iOS'?'📱':'💻'}</div><div><b>${esc(x.device_name||'Trusted device')}</b><small>${esc(x.browser||'Browser')} • ${esc(x.operating_system||'Unknown')}</small><small>Last used ${new Date(x.last_used_at).toLocaleString()}</small><small>Expires ${new Date(x.expires_at).toLocaleDateString()}</small></div><button class="secondary-btn compact-btn" data-remove-device="${x.id}">Remove</button></article>`).join('')||'<div class="security-empty-state"><b>No trusted devices</b><span>The next device you remember will appear here.</span></div>';$$('[data-remove-device]').forEach(b=>b.onclick=()=>removeTrustedDevice(b.dataset.removeDevice))}catch(e){list.innerHTML=`<p class="error-text">${esc(e.message)}</p>`}}
async function removeTrustedDevice(id){if(!confirm('Remove this trusted device? It will need an email code next time.'))return;await api('/.netlify/functions/security-devices',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'remove',id})});localStorage.removeItem(TRUSTED_DEVICE_KEY);trustedDeviceToken='';toast('Trusted device removed');await loadTrustedDevices()}
async function removeAllTrustedDevices(){if(prompt('Type REMOVE ALL to revoke every trusted device.')!=='REMOVE ALL')return;await api('/.netlify/functions/security-devices',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'remove-all'})});localStorage.removeItem(TRUSTED_DEVICE_KEY);trustedDeviceToken='';toast('All trusted devices removed');await loadTrustedDevices()}
...
async function panicLock(){if(prompt('This revokes every active Owner security session. Type PANIC LOCK to continue.')!=='PANIC LOCK')return;await securityAction('panic-lock',{confirmation:'PANIC LOCK'});sessionStorage.removeItem('ase_stepup_token');localStorage.removeItem(TRUSTED_DEVICE_KEY);stepupToken='';trustedDeviceToken='';toast('All Owner security sessions revoked');location.reload()}
...
$('#sendEmailCode').onclick=()=>sendSecurityCode();$('#securityCodeForm').onsubmit=verifySecurityCode;$('#securityResend').onclick=()=>sendSecurityCode();$('#securityRefresh').onclick=loadSecurityProfile;$('#securityBackup').onclick=createSecurityBackup;$('#securityMaintenance').onclick=toggleMaintenance;$('#securityPanic').onclick=panicLock;$('#securityRemoveAllDevices').onclick=removeAllTrustedDevices;$('#maintenanceSetupForm').onsubmit=submitMaintenanceSetup;$$('[data-maintenance-setup-close]').forEach(x=>x.onclick=closeMaintenanceSetup);$('#maintenanceResume').onclick=resumeOperations;$('#maintenanceLockLogout').onclick=logout;
