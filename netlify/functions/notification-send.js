...
exports.handler=async event=>{try{
  ...
  const results=await Promise.allSettled(selected.map(r=>sendFCM(r,payload)));
  const invalidTokens=[];results.forEach((res,i)=>{if(res.status==='rejected'&&/UNREGISTERED|not found|registration-token-not-registered/i.test(res.reason?.message||''))invalidTokens.push(selected[i].token)});
  ...
  const record={id,title,body,audience:b.audience||'roles',roles,users,priority:b.priority||'normal',url:b.url||'/ops/',createdAt:new Date().toISOString(),createdBy:{id:actor.user.id,email:actor.user.email,name:actor.user.user_metadata?.full_name||actor.user.email,role:actor.role},targeted:selected.length,sent,failed};
