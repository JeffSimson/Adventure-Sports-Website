...
async function verifiedUser(event){
  ...
  if(bootstrap.includes(String(user.email||'').toLowerCase()))role='owner';
  return {user,role:VALID_ROLES.includes(role)?role:null,authorization};
}
function requireRole(actor,allowed){if(!actor.role)throw error('Your account has no assigned app role.',403);if(!allowed.includes(actor.role))throw error('You do not have permission to use this feature.',403)}
...
async function adminFetch(path,context,options={}){
  const token=adminToken(context);if(!token)throw error('Identity administration is not configured. See README-V2.txt.',500);
  const response=await fetch(identityBase()+path,{...options,headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json',...(options.headers||{})}});
  const raw=await response.text();let data={};if(raw){try{data=JSON.parse(raw)}catch{data={message:raw.slice(0,300)}}}
...
async function effectiveRole(user){
  const bootstrap=[process.env.OWNER_EMAIL,...String(process.env.OWNER_EMAILS||'').split(',')].filter(Boolean).map(x=>x.trim().toLowerCase());
  if(bootstrap.includes(String(user.email||'').toLowerCase()))return 'owner';
  const r=String(user.app_metadata?.role||'').toLowerCase();
  return VALID_ROLES.includes(r)?r:null;
}
module.exports={json,error,VALID_ROLES,verifiedUser,requireRole,adminFetch,effectiveRole};
