...
async function sendFCM(registration,payload){
  const auth=await accessToken();
  const body={message:{token:registration.token,notification:{title:payload.title,body:payload.body},data:{
    url:String(payload.url||'/ops/'),notificationId:String(payload.notificationId||''),priority:String(payload.priority||'normal')
  },webpush:{headers:{Urgency:payload.priority==='emergency'?'high':'normal'},notification:{
  ...
  },fcm_options:{link:payload.url||'/ops/'}}}};
  ...
  const data=await response.json().catch(()=>({}));if(!response.ok)throw Object.assign(new Error(data?.error?.message||`Firebase send failed (${response.status}).`),{code:data?.error?.status});
  return data;
}
