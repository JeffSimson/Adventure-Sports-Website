...
const fail=(m,s=500)=>Object.assign(new Error(m),{statusCode:s});
...
const offset=d=>{const z=new Intl.DateTimeFormat('en-US',{timeZone:'America/New_York',timeZoneName:'longOffset'}).formatToParts(new Date(d+'T12:00:00Z')).find(x=>x.type==='timeZoneName')?.value||'GMT-04:00';return z.replace('GMT','')};
function range(e){const q=e.queryStringParameters||{},r=(q.range||'today').toLowerCase(),today=dateET();let s=today,n=today,l='Today';if(r==='yesterday'){s=n=add(today,-1);l='Yesterday'}else if(r==='week'){s=weekStart(today);l='This Week'}else if(r==='month'){s=today.slice(0,7)+'-01';l='This Month'}else if(r==='custom'){s=q.start;n=q.end;if(!/^\d{4}-\d{2}-\d{2}$/.test(s||'')||!/^\d{4}-\d{2}-\d{2}$/.test(n||''))throw fail('Choose valid custom dates.',400);if(s>n)throw fail('Start date must be before end date.',400);if((Date.parse(n)-Date.parse(s))/86400000>92)throw fail('Custom reports are limited to 93 days.',400);l=`${s} to ${n}`}return{preset:r,label:l,startDate:s,endDate:n,start:Date.parse(`${s}T00:00:00${offset(s)}`),end:Date.parse(`${n}T23:59:59.999${offset(n)}`)}}
const gate=n=>/front\s*gate/i.test(String(n||''));
const lineName=l=>l?.name||l?.item?.name||'Unnamed item';
const lineGross=l=>l?.refunded||l?.isRevenue===false?0:Math.max(0,num(l.price)*Math.max(0,num(l.unitQty||1000)/1000));
const lineDiscount=l=>els(l?.discounts).reduce((a,d)=>a+Math.abs(num(d.amount)),0);
