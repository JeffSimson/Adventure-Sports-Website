...
const BUILD='9120',VERSION='9.1.2';
...
async function checkAutomation(){const started=Date.now();try{const settings=await getStoreValue('ase-automation','settings',{}),state=await getStoreValue('ase-automation','state',{sent:{}}),last=state.lastRunAt||state.lastSuccessAt||null,age=last?Date.now()-new Date(last).getTime():null;let status='pass',detail='Automation is enabled and state storage is available.';if(settings.enabled===false){status='warn';detail='Automatic operations alerts are currently disabled in Settings.'}else if(!last){status='warn';detail='Automation is enabled, but no completed scheduled run is recorded yet.'}else if(age>12*60000){status='warn';detail=`Last recorded run was ${Math.round(age/60000)} minutes ago; scheduled checks should run every 5 minutes.`}else detail=`Last run completed ${Math.max(0,Math.round(age/60000))} minute(s) ago. ${Object.keys(state.sent||{}).length} dedupe key(s) are stored.`;return (status==='pass'?good:warn)('automation','Automatic operations alerts',detail,elapsed(started),{lastRunAt:last,lightning:state.lightning||null,dedupeKeys:Object.keys(state.sent||{}).length})}catch(e){return bad('automation','Automatic operations alerts',e.message,elapsed(started))}}
...
async function checkWorkOrders(){const started=Date.now();try{const {data}=await supabase('work_orders?select=id,status,due_at&status=not.in.(completed,closed,cancelled)&limit=100');const rows=Array.isArray(data)?data:[],overdue=rows.filter(x=>x.due_at&&new Date(x.due_at)<new Date()).length;return good('work-orders','Work orders',`${rows.length} open work order(s); ${overdue} overdue.`,elapsed(started),{open:rows.length,overdue})}catch(e){return warn('work-orders','Work orders',`Diagnostics could not read work orders: ${e.message}`,elapsed(started))}}
async function checkOperationsCenter(){
 ...
 try{
  ...
  const [matrix,settings,workResult]=await Promise.all([
   ...
   supabase('work_orders?select=id,status,due_at&status=not.in.(completed,closed,cancelled)&limit=100').catch(()=>({data:[]}))
  ]);
  ...
  if(!day)return warn('operations-center','Operations Center data',`The live matrix has no day matching ${today}; game cards will correctly show zero for today.`,elapsed(started),{todayGames:0,fieldsInUse:0,fieldsOpeningSoon:0});
  ...
  const work=Array.isArray(workResult.data)?workResult.data:[],overdue=work.filter(x=>x.due_at&&new Date(x.due_at)<now).length;
